...
class CustomizeManager:
    ...
    def __init__(self):
        ...
        self.selected_slot_idx = 0  # 0:head, 1:r_arm, 2:l_arm, 3:legs
        self.selected_part_list_idx = 0
        
        self.slots = ["head", "right_arm", "left_arm", "legs"]
    ...
    def _handle_slot_select(self, input_comp):
        if input_comp.key_up:
            self.selected_slot_idx = (self.selected_slot_idx - 1) % 4
        elif input_comp.key_down:
            self.selected_slot_idx = (self.selected_slot_idx + 1) % 4
        ...
        # 左右キーでクイックパーツ切り替え
            ...
            current_part_id = current_setup["parts"][slot_name]
            
            new_part_id = self.parts_manager.get_next_part_id(current_part_id, direction)
            self.save_data.update_part(self.selected_machine_idx, slot_name, new_part_id)
    ...
    def get_ui_data(self):
        ...
        slot_name = self.slots[self.selected_slot_idx]
        focused_part_id = setup["parts"][slot_name]
        
        # リスト選択中の場合はリストのものをフォーカス
        if self.state == self.STATE_PART_LIST_SELECT:
            available_ids = self.parts_manager.get_part_ids_for_type(slot_name)
            focused_part_id = available_ids[self.selected_part_list_idx]
        ...
        return {
            ...
            "parts_setup": setup["parts"],
            "focused_part_id": focused_part_id,
            "focused_part_data": self.parts_manager.get_part_data(focused_part_id),
            "available_parts_ids": self.parts_manager.get_part_ids_for_type(slot_name)
        }